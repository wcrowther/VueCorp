...
const sendMessage = async () => 
{
	...
	{
		await connection.invoke('SendMessage', firstName, message.value);
		message.value = ''
	}
};
...
onMounted(async () => 
{
	...
	{
		messages.value.push({ userName, text: message })
	});
		...
		<div class="p-4 w-full md:w-1/2">
			...
			<div :class="['flex', (latestFirst ? 'flex-col-reverse' : 'flex-col')]">
				<div v-for="(msg, index) in messages" :key="index"
					:class="['p-2 pl-0', {'ml-5 text-red' : msg.userName !== firstName}]">
