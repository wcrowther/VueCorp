	...
	private static Action<SwaggerGenOptions> AddMySwaggerGenOptions()
	{
		return options =>
		{
			options.SwaggerDoc("v1", new OpenApiInfo
			{
				...
				"Most endpoints require authorization — log in via Authenticate.Login below, and copy the JWT token then click on " +
				"the Authorization button below and follow directions to insert into the field. "
			});
			...
			options.AddSecurityDefinition("Bearer",
				new OpenApiSecurityScheme()
				{
					...
					Name            = "Authorization",
					BearerFormat    = "JWT",
					Description     = "Call the 'Login' endpoint to get a JWT token. Add the token (prefixed with 'Bearer '), " +
